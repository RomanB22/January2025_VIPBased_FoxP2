...
for factorAmpAux in [0.5,1,2]:
    for factorWidthAux in [1]:
        for Condition in ['OnlyIncre','MirrorDecre','InVivo']:


            with open("spikes/DataDepol%sFit%sA_%sW%s.pkl" % (DepolBlock, Fit, factorAmpAux, factorWidthAux), 'rb') as f:
            ...
            plt.savefig("figuresNoDepol/FoxP2Time_%dDepol%sFit%sA_%sW%s.eps" % (PT5BInc, DepolBlock, Fit, factorAmpAux, factorWidthAux))
            ...
            plt.legend(frameon=False)
            plt.savefig("figuresNoDepol/Impedance_%dDepol%sFit%sA_%sW%s.eps" % (PT5BInc, DepolBlock, Fit, factorAmpAux, factorWidthAux))
            # plt.show()
            plt.close()
            ...
            plt.legend(frameon=False)
            plt.savefig("figuresNoDepol/Rates%s_%dDepol%sFit%sA_%sW%s.eps" % (Condition, PT5BInc, DepolBlock, Fit, factorAmpAux, factorWidthAux))
            # plt.show()
            plt.close()
            ...
            plt.legend(frameon=False)
            plt.savefig("figuresNoDepol/BackgroundRates_%dDepol%sFit%sA_%sW%s.eps" % (PT5BInc, DepolBlock, Fit, factorAmpAux, factorWidthAux))
            # plt.show()
            plt.close()
            ...
            for Condition in conditionCoord:
                ...
                for Ntotal in nTotalCoord[1:4]:
                    ...
                    for PT5BInc in pt5b_incCoord:
                        ...
                        dataPeaks50percent = data2plotAux['FoxP2Rate'] - (1-0.60)*np.mean(data2Baseline['FoxP2Rate'])
                        dataPeaks50percent = dataPeaks50percent.values

                        import math
                        if math.isnan(np.unique(dataPeaks)[0]) or math.isnan(np.unique(dataPeaks50percent)[0]): continue

                        baselineDistanceDict[str(Condition)][str(Ntotal)][str(PT5BInc)] = dataPeaks[-1]#/np.mean(data2Baseline['FoxP2Rate'])

                        peaks, properties = find_peaks(dataPeaks, prominence=None, width=10, height=0)
                        peaks50percent, properties50percent = find_peaks(dataPeaks50percent, prominence=None, width=10, height=0)

                        if len(peaks)==0 or len(peaks50percent)==0:
                            continue

                        x = data2plotAux['FoxP2Rate'].values

                        zero_crossings = np.where(np.diff(np.sign(x)))[0]
                        zero_crossings50percent  = np.where(np.diff(np.sign(dataPeaks50percent)))[0]
                        ...
                        try:
                            peak_widthRight = zero_crossings50percent[[i>peaks50percent[0] for i in zero_crossings50percent]][0]+1
                        except:
                            continue
                        ...
                        # plt.plot(timeCoord, dataPeaks)
                        # plt.plot(timeCoord[peak_widthLeft], dataPeaks[peak_widthLeft], 'x')
                        # plt.plot(timeCoord[peak_widthRight], dataPeaks[peak_widthRight], 'x')
                        # plt.show()
            ...
            for Condition in conditionCoord:
                for i in df[Condition].index:
                    percentagePT5B = []
                    returnTime = []
                    for key, value in df[Condition][i].items():
                        # print(key, value)
                    ...
                    if len(returnTime)==0: continue
                    plt.plot(percentagePT5B, returnTime, marker=marker[Condition], color=color[i], linewidth=0, label=Condition+" - N="+str(i))
            plt.legend(frameon=False)
            plt.xlabel("%PT5B_inc")
            plt.xlim(-5,105)
            plt.ylabel("Return to %60 of baseline  (ms)")
            ax = plt.gca()
            ax.spines['top'].set_visible(False)
            ax.spines['right'].set_visible(False)
            plt.savefig("figuresNoDepol/ReturnToBaselineDepol%sFit%sA_%sW%s.eps" % (DepolBlock, Fit, factorAmpAux, factorWidthAux))
            plt.close()
            # plt.show()
            ...
            for i in dfDistance['InVivo'].index[indexAux]:
                ...
                percentagePT5B = np.array(percentagePT5B).flatten()
                returnTime = np.array(returnTime).flatten()
                plt.plot(percentagePT5B, returnTime, marker='o', color=color[i], linewidth=0.5, label="In Vivo - N="+str(i))
            ...
            for i in dfDistance['MirrorDecre'].index[indexAux]:
                ...
                percentagePT5B = np.array(percentagePT5B).flatten()
                returnTime = np.array(returnTime).flatten()
                plt.plot(percentagePT5B, returnTime, marker='x', color=color[i], linewidth=0.5, label="Not decreasing PT_dec - N="+str(i))
            ...
            for i in dfDistance['OnlyIncre'].index[indexAux]:
                ...
                percentagePT5B = np.array(percentagePT5B).flatten()
                returnTime = np.array(returnTime).flatten()
                plt.plot(percentagePT5B, returnTime, marker='s', color=color[i], linewidth=0.5, label="Only PT_inc- N="+str(i))
            plt.legend(frameon=False)
            ...
            ax.spines['top'].set_visible(False)
            ax.spines['right'].set_visible(False)
            plt.savefig("figuresNoDepol/DistanceToBaselineDepol%sFit%sA_%sW%s.eps" % (DepolBlock, Fit, factorAmpAux, factorWidthAux))
            plt.close()
            # plt.show()
