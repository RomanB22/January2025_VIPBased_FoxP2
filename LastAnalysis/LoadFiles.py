...
Fit = False
constantArea = False
folderLoad = '../dataExpInputs'
folderSave = 'resultsExpInputs'

Path(folderSave).mkdir(parents=True, exist_ok=True)

for factorAmpAux in [0.25, 0.5, 0.75, 1, 1.25, 1.5, 1.75, 2]:
    for factorWidthAux in [1.0]:#[0.5,1.,2.]:
        folder = folderLoad+'/g_F%s_A%s' % (str(Fit)[0], str(constantArea)[0])
        ...
        for file in files:
            ...
            factorAmp = data['simConfig']['factorAmp']
            factorWidth = data['simConfig']['factorWidth']
            Fitting = data['simConfig']['Fitting']

            if abs(factorAmp-factorAmpAux)>=1e-3 or abs(factorWidth-factorWidthAux)>=1e-3: continue

            FileName = '%s_%d_%d_%d_%s_%s_%s_%s_%s_%s_%s_%s_%s_%s' % (Condition, NumIncreasing, NumDecreasing, NumNotRelated,
                                                                      NetStimPre, NetStimPost, NetStimNoise, AMPAWeight,
                                                                      SynsPerConn, IAmp, NoisePresent, factorAmp, factorWidth,
                                                                      Fitting)
        ...
        with open(folderSave+"/SimSpikesFit%sA_%sW%s_Area%s.pkl" % (Fit, factorAmpAux, factorWidthAux, constantArea), 'wb') as f:
